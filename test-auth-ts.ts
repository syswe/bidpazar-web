...
import { register, login, validateToken, getToken, getUser } from './src/lib/frontend-auth';
...
async function testBackendConnection() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Backend connection failed: ${response.status} ${JSON.stringify(data)}`);
    }
