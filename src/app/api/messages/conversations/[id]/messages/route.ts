...
export async function GET(
  ...
  console.log(
    `[API][${urlPath}] GET request received. Query:`,
  ...
  try {
    ...
    console.log(
      `[API][${urlPath}] Extracted conversationId: ${conversationId}`
    );
    const page = parseInt(queryParams.get("page") || "1");
    const limit = parseInt(queryParams.get("limit") || "20");
    console.log(
      `[API][${urlPath}] Using pagination - Page: ${page}, Limit: ${limit}`
    );

    if (!conversationId) {
      console.warn(
        `[API][${urlPath}] Bad Request (400): Conversation ID parameter is missing.`
      );
    ...
    if (!conversation) {
      console.error(
        `[API][${urlPath}] Forbidden (403): User is not a participant in this conversation`
      );
      return NextResponse.json(
        {
    ...
    console.log(
      `[API][${urlPath}] Conversation ID: ${conversationId}, Total Messages: ${totalCount}`
    );
    ...
    const messages = await prisma.message.findMany({
      ...
      orderBy: {
        createdAt: "asc", // Changed from 'desc' to 'asc' to show oldest messages first
      },
      skip: (page - 1) * limit,
      take: limit,
    });
    ...
    console.log(
      `[API][${urlPath}] Successfully fetched ${messages.length} messages from database`,
      messages.map((m) => ({
        id: m.id,
