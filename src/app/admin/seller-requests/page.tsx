      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Toplam Başvuru</h3>
            <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">{requests.length}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Bekleyen</h3>
            <p className="text-3xl font-bold text-yellow-600 dark:text-yellow-400">
              {requests.filter(r => r.status === 'PENDING').length}
            </p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Onaylanan</h3>
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Satıcı Başvuruları</h3>
          </div>
                        ...
                        {request.status === 'PENDING' ? (
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              ...
                              >
                                {processing === request.id ? 'İşleniyor...' : 'Onayla'}
                              </button>
                              <button
                              ...
                              >
                                {processing === request.id ? 'İşleniyor...' : 'Reddet'}
                              </button>
                            </div>
                          </div>
                        ) : (
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            {request.reviewedAt && (
                              <div>
                                {new Date(request.reviewedAt).toLocaleDateString('tr-TR')} tarihinde değerlendirildi
                                {request.reviewNotes && (
        ...
        {requests.some(r => r.notes) && (
          <div className="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Başvuru Notları</h3>
            </div>
            <div className="p-6 space-y-4">
              {requests.filter(r => r.notes).map((request) => (
                <div key={request.id} className="border-l-4 border-blue-500 pl-4">
                  <div className="font-medium text-gray-900 dark:text-white">
                    {request.fullName} (@{request.user.username})
                  </div>
