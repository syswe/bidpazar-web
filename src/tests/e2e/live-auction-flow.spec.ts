...
test("Live stream auction flow: streamer, watchers, auction, bids", async ({
  browser,
}) => {
  ...
  await streamerPage.waitForSelector("text=Yayına Başlamaya Hazır Mısın?", {
    timeout: 30000,
  });
  ...
  const watcherContexts = await Promise.all(
    [1, 2, 3].map(() => browser.newContext())
  );
  const watcherPages = await Promise.all(
    watcherContexts.map((ctx) => ctx.newPage())
  );
